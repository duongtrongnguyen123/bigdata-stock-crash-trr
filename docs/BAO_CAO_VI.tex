...
\textbf{Giảng viên giảng dạy:}\\ TS. Trần Hồng Việt \quad ThS. Ngô Minh Hương\\\vspace{0.5cm}\\
...
\section*{Phân công nhiệm vụ thành viên}
\addcontentsline{toc}{section}{Phân công nhiệm vụ thành viên}
Nhóm gồm hai thành viên; khối lượng công việc được phân chia dựa trên vai trò và đóng góp thực tế:
...
\begin{table}[H]
    ...
    \toprule
    \textbf{Thành viên} & \textbf{Công việc đảm nhận} \\
    \midrule
    \textbf{Dương Trọng Nguyên} &
    \begin{itemize}[leftmargin=1.2em, nosep]
        \item Thiết kế và cài đặt thuật toán TRR 4 pha (Brainstorm $\rightarrow$ Memory $\rightarrow$ Attention $\rightarrow$ Reason) và module Causal RAG.
        \item Xây dựng pipeline Big Data: stream-filter corpus FNSPID 23 GB, Spark ETL $\rightarrow$ Parquet, lập chỉ mục SQLite theo ngày.
        \item Triển khai phân tán mô hình 32B trên Kaggle (fan-out 40 shard); đánh giá AUROC, bootstrap significance, severity correlation và backtest kinh tế.
        \item Tổng hợp và viết báo cáo (chính).
    \end{itemize} \\
    \midrule
    \textbf{Nguyễn Ngọc Bình An} &
    \begin{itemize}[leftmargin=1.2em, nosep]
        \item Xây dựng tầng phục vụ thời gian thực: mô hình 7B-AWQ cục bộ, daemon live, API FastAPI và dashboard Streamlit.
        \item Thu thập \& tiền xử lý dữ liệu (yfinance, Google News RSS); sinh nhãn crash từ giá tương lai.
        \item Hiệu chuẩn xác suất (Isotonic), Precision@K và kết xuất hình ảnh/biểu đồ kết quả.
        \item Chuẩn bị slide thuyết trình và video demo.
    \end{itemize} \\
    \bottomrule
    \end{tabularx}
\end{table}
...
\subsection{Giao diện Demo (Web)}
Toàn bộ hệ thống được đóng gói thành một ứng dụng web \textbf{Streamlit}. Tab \textit{Live \& Advisory} (Hình \ref{fig:web_live}) hiển thị xác suất crash thời gian thực kèm mức rủi ro và lý do; tab \textit{Research \& Backtest} (Hình \ref{fig:web_research}) trình bày các lần chạy lịch sử có nhãn với AUROC, dòng thời gian và phát lại crash radar theo ngày.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth,keepaspectratio]{../reports/figures/fig_web_live.png}
    \caption{Giao diện \textit{Live \& Advisory}: gauge xác suất crash, mức rủi ro và lý do (mô hình Qwen2.5-32B, hiển thị giờ Việt Nam).}
    \label{fig:web_live}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth,keepaspectratio]{../reports/figures/fig_web_research.png}
    \caption{Giao diện \textit{Research \& Backtest}: một lần chạy có nhãn (COVID 2020) với AUROC, số ngày crash thực tế và phát lại crash radar theo từng ngày.}
    \label{fig:web_research}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth,keepaspectratio]{../reports/figures/fig_spark_ui.png}
    \caption{Spark Web UI --- tab Executors (cổng \texttt{:4040}) khi chạy ETL \texttt{corpus-etl}: một executor \texttt{local[*]} với \textbf{8 cores} đang xử lý phân tán (Active/Total Tasks, Input 1,1 MiB, Task Time 53 s).}
    \label{fig:spark_ui}
\end{figure}

\begin{lstlisting}[numbers=none, caption={Data-lake Parquet phân vùng theo năm: 12 GB CSV $\rightarrow$ 718 MB; \textit{partition pruning} chỉ quét đúng một thư mục năm.}, label={lst:parquet}]
data/lake/news_parquet/      718 MB   (Parquet, from 12 GB CSV)
    year=2016/   62 MB
    year=2017/   63 MB
    year=2018/   66 MB
    year=2019/   58 MB
...
# partition pruning:  SELECT * FROM news WHERE year = 2020
#   -> reads ONLY 1 partition (351,832 rows) in 0.1s, not the whole lake
\end{lstlisting}
...
\begin{table}[H]
    ...
    \toprule
    \textbf{Bộ dữ liệu} & \textbf{Khoảng tg} & \textbf{\#mã} & \textbf{\#bài} & \textbf{\#ngày} & \textbf{\#crash} & \textbf{Tần suất} \\
    \midrule
